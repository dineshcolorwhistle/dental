...
export class FinanceService {
  ...
  async getFinanceStats(
    ...
    try {
      ...
      // If branch filter is 'NONE', return empty stats (no branch context available)
      if (branchIdsFilter === 'NONE') {
        return this.emptyFinanceStats();
      }
      ...
      console.error(
        `Error in FinanceService.getFinanceStats: ${error?.message || error}`,
        error?.stack,
      );
      return this.emptyFinanceStats();
    }
  ...
  async getPendingPayments(
    ...
    try {
      ...
      // If branch filter is 'NONE', return empty results (no branch context available)
      if (branchIdsFilter === 'NONE') {
      ...
      console.error(
        `Error in FinanceService.getPendingPayments: ${error?.message || error}`,
        error?.stack,
      );
      return { data: [], meta: { total: 0, page, limit, totalPages: 0 } };
    }
  ...
  private emptyFinanceStats() {
    return {
      summary: {
        ...
        topPerformingBranch: { name: 'N/A', revenue: 0 },
        pendingPaymentCount: 0,
      },
      branchPerformance: [],
      monthlyTrends: [],
      paymentStatusDistribution: { paidCount: 0, pendingCount: 0 },
    };
  }
}
